...
def get_movies(dom):
    ...
    movies = []
    rows = soup.find_all('ul', {'class': 'release_list'})
    for row in soup.find('ul', 'release_list').find_all('li'):
        movie = dict()
        #movie['expectation'] = row.find(id='ymvle').find('div', 'bd clearfix ').em.text
        movie['ch_name'] = row.find('div', 'release_movie_name').a.text.replace('\n','').strip()
        movie['en_name_'] = row.find('div','en').text.replace('\n','').strip()
        movie['love_'] = row.find('div','leveltext').span.text.replace('\n','').strip()
        movie['release_date'] = row.find('div','release_movie_time').text.replace('\n','').strip()
